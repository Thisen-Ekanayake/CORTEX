...
def log_metrics(y_true, y_pred, split_name, step=None):
    ...
    precision_pc, recall_pc, f1_pc, _ = precision_recall_fscore_support(
        y_true, y_pred, average=None, zero_division=0, labels=sorted(LABEL_NAMES.keys())
    )

    for idx, class_id in enumerate(sorted(LABEL_NAMES.keys())):
        metrics[f"{split_name}/{LABEL_NAMES[class_id]}_precision"] = precision_pc[idx]
        metrics[f"{split_name}/{LABEL_NAMES[class_id]}_recall"] = recall_pc[idx]
        metrics[f"{split_name}/{LABEL_NAMES[class_id]}_f1"] = f1_pc[idx]

    wandb.log(metrics, step=step)

    # Confusion matrix (consistent class order)
    wandb.log({
...
def train_and_save(data_dir, out_dir="model", test_size=0.2, val_size=0.1, random_seed=42):
    ...
    # Load data (set require_all_files=True once you created rag_doc.txt and rag_img.txt)
    texts, labels = load_data(data_dir, require_all_files=False)

    print(f"Total samples: {len(texts)}")
    wandb.log({"total_samples": len(texts)})

    unique, counts = np.unique(labels, return_counts=True)
    class_dist = {LABEL_NAMES[label]: int(count) for label, count in zip(unique, counts)}
    ...
    wandb.log({"train_size": len(X_train), "val_size": len(X_val), "test_size": len(X_test)})
