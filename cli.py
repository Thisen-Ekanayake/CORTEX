...
from cortex.router import Route, execute, route_query
...
def print_confidence_scores(scores: dict[str, float], predicted_route: Route):
    ...
    for route_name, confidence in sorted_scores:
        ...
        if route_name == predicted_route.value:
            print(
                f"  {bold}{color}► {route_name.upper():<6}{reset} {color}{bar}{reset} {bold}{confidence:.1%}{reset}"
            )
        else:
...
def print_response_header(route: Route, confidence: float):
    ...
    print(
        f"\n{bold}{color}[{route_name}]{reset} {color}({confidence:.1%}){reset} ",
        end="",
        flush=True,
    )
