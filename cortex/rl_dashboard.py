...
from cortex.rl_router import RoutingFeedback, get_rl_router


def print_ascii_chart(
    data: list[float], height: int = 10, width: int = 50, title: str = "Accuracy Over Time"
):
    """
    ...
    print("       0" + " " * (len(normalized) - 10) + f"{len(data)}")
...
def print_route_performance(rl_router):
    ...
    for route_name in ["rag", "rag_doc", "rag_img", "meta", "chat"]:
        ...
        print(f"{route_name.upper():6} │ {color}{bar}\033[0m {acc:5.1%} ({correct}/{total})")
...
def print_recent_mistakes(rl_router, n: int = 5):
    ...
    for i, fb in enumerate(mistakes, 1):
        ...
        print(
            f"   Predicted: \033[91m{fb.predicted_route.upper()}\033[0m "
            f"({pred_conf:.1%} confidence)"
        )
        print(
            f"   Actual:    \033[92m{fb.actual_route.upper()}\033[0m ({actual_conf:.1%} confidence)"
        )
