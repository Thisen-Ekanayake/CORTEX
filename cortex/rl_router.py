    ...
    def _learn_from_feedback(self, feedback: RoutingFeedback):
        ...
        if feedback.correct:
            ...
            self.confidence_weights[feedback.actual_route] = min(
                2.0, current + self.learning_rate * feedback.reward
            )
        else:
            ...
            self.confidence_weights[feedback.actual_route] = min(
                2.0, current_correct + self.learning_rate * abs(feedback.reward) * 0.5
            )
    ...
    def get_metrics(self) -> dict:
        ...
        for route, stats in self.route_accuracy.items():
            if stats["total"] > 0:
                ...
                route_metrics[route] = {"accuracy": 0.0, "total": 0, "correct": 0}
...
def get_rl_router(feedback_dir: str = "rl_feedback", learning_rate: float = 0.1) -> RLRouter:
    """
    ...
    if _rl_router is None:
        ...
        _rl_router = RLRouter(
            base_router=base_router, feedback_dir=feedback_dir, learning_rate=learning_rate
        )
    return _rl_router
